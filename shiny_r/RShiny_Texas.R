...
ui <- fluidPage(
  titlePanel("Binge Drinking Age-adjusted Prevalence in TX by County"),
  sidebarLayout(
    sidebarPanel(
...
server <- function(input, output, session) {
  ...
  # Load the dataset
  df <- reactive({
    url <- "https://raw.githubusercontent.com/meglee67/datasci_4_web_viz/main/datasets/PLACES__Local_Data_for_Better_Health__County_Data_2023_release.csv"
    read.csv(url)
  })
